...
\end{figure}
To provide a quantitative validation, Table~\ref{parametros} details the time-domain performance metrics and steady-state error alongside the integrated error indices (IAE, ISE, ITAE) and the control effort metric (ISSC).
...
\setlength{\tabcolsep}{4pt} % Espaçamento horizontal elegante para 11 colunas
...
To evaluate the robustness, recovery capability, and disturbance rejection of the proposed control architectures, a step reference of ($\pm20^\circ$) was injected at $t = 10\text{ s}$ from a steady state of repose, followed by the application of an external physical disturbance once the system reached its steady-state operating point. The real-time angular displacement tracking performance under these perturbed conditions is illustrated in Fig.~\ref{fig:disturbance_response}. Concurrently, the respective actuator efforts, representing the generated control signals during the recovery and stabilization process, are presented in Fig.~\ref{fig:disturbance_control_signals}.
\begin{figure}[htbp]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \centering
...
To provide a quantitative validation under perturbed conditions, Table~\ref{tab:performance_metrics_disturbance} details the time-domain performance metrics alongside the integrated error indices (IAE, ISE, ITAE) and the energetic control effort (ISSC).
