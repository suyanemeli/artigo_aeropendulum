...
\author*[1]{\fnm{Suyane Melissa} \sur{Soares Florencio}}\email{suya1n2e@gmail.com}

\author[1]{\fnm{Reinel} \sur{Beltrán Aguedo}}\email{reinel.beltran@ufrpe.br}
\equalcont{These authors contributed equally to this work.}
\author[1]{\fnm{Ania} \sur{Lussón Cervantes}}\email{ania.lusson@ufrpe.br}
\equalcont{These authors contributed equally to this work.}
\affil*[1]{\orgdiv{Unidade Acadêmica do Cabo de Santo Agostinho (UACSA)}, \orgname{Universidade Federal Rural de Pernambuco (UFRPE)}, \orgaddress{\city{Cabo de Santo Agostinho}, \state{Pernambuco}, \country{Brazil}}}

%%==================================%%
%% Sample for unstructured abstract %%
%%==================================%%

\abstract{This paper presents the design, modeling, and experimental validation of a low-cost twin-rotor aeropendulum testbed to evaluate the impact of low-level actuation on aerospace control systems. Standard brushless propulsion units traditionally drive the plant via voltage-equivalent signals (PWM), introducing severe dynamic non-linearities and thrust asymmetries. To address these issues, we propose and compare two low-level operational frameworks: a conventional equivalent voltage-driven scheme and a software-based virtual estimated current loop. Both frameworks are benchmarked under classical parallel PID and adaptive hybrid Fuzzy-PID position controllers. Experimental results demonstrate that the virtual current loop successfully linearizes the actuation channel, offering superior dynamic damping and reducing the control energy index (ISSC) by up to 70\% compared to the voltage-driven approach. Under external disturbances, the current-driven paradigm exhibits enhanced rejection capabilities and superior steady-state precision. Furthermore, while the current-driven Fuzzy-PID achieves the best performance-to-energy trade-off for positive setpoints, the conventional PID with virtual current actuation proves to be the most balanced and reliable strategy at negative operating points.}
...
Among the real-world applications requiring high technical rigor, Unmanned Aerial Vehicles (UAVs) stand out. These multi-rotor platforms rely on robust control strategies to guarantee flight stability, precise trajectory tracking, and external disturbance rejection across diverse sectors, including precision agriculture \citep{anziliero2021tecnicas} and environmental monitoring \citep{policarpo2025camera}. To advance UAV autonomy, recent literature has focused heavily on high-level navigation and guidance control loops. For instance, real-time collision avoidance strategies using velocity constraints have been validated via augmented reality setups \citep{ricardo2024robust}, tube-based Model Predictive Control (MPC) has been explored for suspended load transportation in ROS/Gazebo environments \citep{andrade2024tube}, and model-free Integral Reinforcement Learning (IRL) has been proposed for multi-quadrotor formation flight \citep{dinh2025model}. Additionally, fuzzy adaptive feedback linearization \citep{aghazamani2025position} and particle swarm optimization-tuned sliding mode control \citep{benayad2025hybrid} have been investigated to handle actuator faults. However, directly evaluating such algorithms on physical aerial platforms presents substantial risks: crash hazards, accelerated electromechanical wear, high cost of motion-capture tracking infrastructure, and embedded microcontroller computational constraints make early-stage experimental testing risky and often unfeasible.
To overcome these practical constraints, laboratory testbeds provide a strategic alternative for evaluating aerospace dynamics. Among them, the aeropendulum is particularly effective. It consists of a rotational arm constrained to a single degree of freedom driven by a propulsion assembly, faithfully reproducing the fundamental thrust-torque-angle interactions present in rotary-wing aircraft \citep{tanveer2023robust, rojas2024hunting}. Consequently, the aeropendulum enables safe and replicable investigation of stability, transient dynamics, and disturbance rejection.

However, the practical effectiveness of high-level control algorithms strictly depends on low-level actuator performance. In propulsion systems based on DC or BLDC motors driven by commercial Electronic Speed Controllers (ESCs), the low-level interface typically operates by mapping the control variable into a voltage signal via Pulse-Width Modulation (PWM) \citep{tanveer2023robust, rojas2024hunting}. Although simple to implement, voltage-driven actuation introduces severe non-linearities stemming from back-electromotive force (back-EMF), thermal resistance variations, and rotor static friction deadbands. Because the aerodynamic thrust and torque governing the plant dynamics derive directly from the electromechanical torque, which bears a linear relationship with electrical current rather than voltage, implementing a current control loop emerges as a compelling alternative to linearize the actuation channel and accelerate transient response.

To bridge this gap, the present work adopts the twin-rotor aeropendulum as an experimental investigation platform. The main contributions of this study are summarized as follows:

\begin{enumerate}
    \item Development and experimental characterization of a software-based virtual current-estimation loop for low-cost BLDC propulsion units, eliminating the need for extra physical current sensors;
    \item Quantitative comparison between equivalent voltage-driven and virtual current-driven paradigms under classical parallel Proportional-Integral-Derivative (PID) and adaptive hybrid Fuzzy-PID architectures;
    \item Comprehensive experimental validation under setpoint regulation and external disturbance rejection, mapping the energy efficiency (ISSC) and transient stability boundaries of each actuation framework.
\end{enumerate}

The remainder of this paper is organized as follows: Section~\ref{sec2} details the mathematical dynamics of the twin-rotor aeropendulum. Section~\ref{sec3} presents the experimental setup and parameter identification. Section~\ref{sec4} describes the voltage and current actuation frameworks. Section~\ref{sec5} presents the design of the classical parallel PID and adaptive Fuzzy-PID controllers. Section~\ref{sec6} discusses the experimental results, and Section~\ref{sec7} concludes the paper.

\section{Mathematical modeling}\label{sec2}

The twin-rotor aeropendulum is modeled as a single-degree-of-freedom rotational system operating in the vertical plane about a fixed, low-friction pivot. The fundamental physical law governing this motion is Newton's second law for rotation, which establishes that the rate of change of angular momentum of a rigid body is equal to the net external torque applied to the system. In this context, the net external torque represents the algebraic sum of the active aerodynamic thrust generated by the motors, the restoring torque due to gravity, and the resistive damping torque from mechanical friction. This resultant torque is directly proportional to the angular acceleration $\ddot{\theta}(t)$, with the constant of proportionality being the total mass moment of inertia $J$, as expressed in Eq.~\ref{eq:newton_rotational}.
\begin{equation}
\sum \tau_i(t) = J \alpha(t) = J \ddot{\theta}(t)
\label{eq:newton_rotational}
\end{equation}

\noindent where $J$ represents the total mass moment of inertia of the rod-motor assembly about its fixed axis of rotation, and $\alpha(t)$ is the angular acceleration. The term $\ddot{\theta}(t)$ serves as an alternative representation of this same angular acceleration, expressing it mathematically as the second-order derivative of the angular displacement $\theta(t)$ with respect to time ($\ddot{\theta}(t) = \frac{d^2\theta(t)}{dt^2}$), where $\theta(t)$ is the angular displacement relative to the vertical equilibrium position. 
...
\begin{figure}[h]
    ...
    \caption{Schematic diagram of the aeropendulum  system.}
    \label{fig:aeropendulum_schematic}
\end{figure}

The structural setup is modeled as a mass $m$ concentrated at the tip of a rigid rod of length $L$, deflected by an angular displacement $\theta$ relative to the vertical equilibrium plane, which is induced in the aeropendulum by external forces through the applied force $F_a$. The gravitational force $F_g = m_{\text{total}}g$, where $g$ denotes the acceleration of gravity, acting on the center of mass is decomposed into two orthogonal components: the radial force $F_{g_{y}} = m_{\text{total}}g\cos\theta$, which acts along the longitudinal axis of the rod and is counterbalanced by the internal structural tension force $T$, and the tangential force $F_{g_{x}} = m_{\text{total}}g\sin\theta$. This tangential component acts perpendicularly to the rod, consistently pulling the system back toward the stable vertical rest position ($\theta = 0^\circ$).
...
\begin{equation}
\tau_{m}(t) + \tau_r(t) + \tau_a(t) = J \ddot{\theta}(t)
...
\section{Experimental platform and parameter identification}\label{sec3}
...
Total moment of inertia ($J$) & 0.0173 & kg$\cdot$m² \\
...
\section{Input signal interpretation and actuation dynamics}\label{sec4}
...
\begin{equation}
K_t = \frac{60}{2\pi K_v} \approx 0.00955 \text{ N}\text{m/A}
\label{eq:kt_value}
\end{equation}
...
\toprule
Actuator &  Function $I(t) = a \text{PWM}(t) + b$ \\
\midrule
Front & $I_{\text{ref}} = 0.006427 \text{PWM}_1(t) - 7.139481$ \\
Rear  & $I_{\text{ref}} = 0.007061 \text{PWM}_2(t) - 7.643550$ \\
\botrule
\end{tabular*}
\end{minipage}
\end{center}
...
\section{Control system design}\label{sec5}

This work proposes and evaluates two distinct control paradigms to regulate the angular position of the twin-rotor aeropendulum: a classical parallel PID controller and an intelligent hybrid Fuzzy-PID controller. Both architectures are independently implemented and compared under both voltage and virtual current actuation frameworks, mapping the performance boundaries of each combination.

\subsection{Classical PID Control Design and Tuning}
\label{sub:pid_design}


\subsection{Classical Parallel PID Controller Synthesis}
\label{sub:pid_synthesis}

The position control system is structured around a classical parallel PID topology. The controller processes the real-time angular position tracking error $e(t) = \theta_{\text{ref}}(t) - \theta(t)$ to generate a scalar control action $u(t)$. In the Laplace domain, the transfer function of the parallel PID controller $C(s)$ is expressed as:
...
where $K_p$, $K_i$, and $K_d$ represent the proportional, integral, and derivative gains, respectively. The resulting control variable $U(s)$ is dispatched as a differential actuation command between the front (actuator 1) and rear (actuator 2) propulsion units to generate the net propulsive moment required for angular displacement.

The synthesis leverages the open-loop transfer functions derived for current actuation ($G_I(s)$) and equivalent voltage actuation ($G_V(s)$), expressed respectively as:

\begin{equation}
G_I(s) = \frac{\Theta(s)}{I_{m1}(s) - I_{m2}(s)} = \frac{K_t / J}{s^2 + \frac{b}{J}s + \frac{K_g}{J}}
\label{eq:tf_current_literal}
\end{equation}

\begin{equation}
G_V(s) = \frac{\Theta(s)}{V_1(s) - V_2(s)} = \frac{(K_1 + K_2) / J}{s^2 + \frac{b}{J}s + \frac{K_g}{J}}
\label{eq:tf_voltage_literal}
\end{equation}

In Eq.~\eqref{eq:tf_current_literal}, the denominator term $\Delta I(s) = I_{m1}(s) - I_{m2}(s)$ represents the net differential armature current between the front ($I_{m1}$) and rear ($I_{m2}$) propulsion units. Because the physical platform lacks embedded hardware current sensors, this differential current command $U(s) = \Delta I(s)$ is synthesized entirely in software.


Similarly, in Eq.~\eqref{eq:tf_voltage_literal}, the input term $\Delta V(s) = V_1(s) - V_2(s)$ denotes the net differential equivalent terminal voltage applied to the Electronic Speed Controllers (ESCs). The gain parameters $K_1$ and $K_2$ correspond to the lumped static voltage-to-torque conversion factors ($K_{\text{equivalent},1}$ and $K_{\text{equivalent},2}$) derived from the steady-state characterization ($\tau(t) = K_{\text{equivalent}}V(t)$), effectively mapping the controller output $U(s)$ into duty cycle commands.
...
\subsection{Adaptive Hybrid Fuzzy-PID Control Design}
\label{sub:fuzzy_pid}
...
\begin{equation}
K_p(t) = \alpha_p(t)  K_{p0}
\label{eq:fuzzy_kp}
\end{equation}
\begin{equation}
K_i(t) = \alpha_i(t)  K_{i0}
\label{eq:fuzzy_ki}
\end{equation}
\begin{equation}
K_d(t) = \alpha_d(t)  K_{d0}
\label{eq:fuzzy_kd}
\end{equation}
